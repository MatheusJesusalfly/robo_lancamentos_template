...
A pendencia e por conversa: duas pessoas falando com o robo ao mesmo tempo nao
se misturam.
...
class Conversa:
    def __init__(self, interpretador: Interpretador, destinos: list[Destino],
        ...
        self._config = config
        #: conversa -> (texto acumulado, ultimo lancamento montado)
        self._pendentes: dict[int, tuple[str, Lancamento]] = {}
    ...
    def receber(self, msg: Mensagem) -> str | None:
        ...
        baixo = texto.lower()
        pendente = self._pendentes.get(msg.conversa)

        if pendente:
            if baixo in SIM:
                return self._gravar(msg.conversa)
            if baixo in NAO:
                self._pendentes.pop(msg.conversa)
            ...
            return self._montar(msg.conversa, f"{pendente[0]}\n\nCorrecao: {texto}")

        if not _TEM_VALOR.search(texto):
            return AJUDA

        return self._montar(msg.conversa, texto)

    def _montar(self, conversa: int, texto: str) -> str:
        try:
        ...
        self._pendentes[conversa] = (texto, lanc)
        return resumir(lanc, conferir(lanc))

    def _gravar(self, conversa: int) -> str:
        _, lanc = self._pendentes[conversa]
        ...
        return "\n".join(linhas) if linhas else "Nao consegui gravar em lugar nenhum."
